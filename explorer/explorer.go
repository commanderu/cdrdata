...
type explorerDataSourceLite interface {
	...
	DecodeRawTransaction(txhex string) (*cdrjson.TxRawResult, error)
...
func (exp *explorerUI) Store(blockData *blockdata.BlockData, _ *wire.MsgBlock) error {
	...
	exp.ExtraInfo = &HomeInfo{
		...
		PoolInfo: TicketPoolInfo{
			...
			Percentage: percentage(blockData.PoolInfo.Value, cdrutil.Amount(blockData.ExtraInfo.CoinSupply).ToCoin()),
		...
		TicketROI: func() float64 {
			PosSubPerVote := cdrutil.Amount(blockData.ExtraInfo.NextBlockSubsidy.PoS).ToCoin() / float64(exp.ChainParams.TicketsPerBlock)
		...
		// 1.  Not all blocks have 5 votes.  On average each block in commanderu
