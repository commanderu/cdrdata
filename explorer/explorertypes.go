...
type AddressInfo struct {
	// Address is the commanderu address on the current page
	Address string
	...
	AmountReceived  cdrutil.Amount
	AmountSent      cdrutil.Amount
	AmountUnspent   cdrutil.Amount
...
// BlockSubsidy is an implementation of cdrjson.GetBlockSubsidyResult
...
func ReduceAddressHistory(addrHist []*dbtypes.AddressRow) *AddressInfo {
	...
	for _, addrOut := range addrHist {
		coin := cdrutil.Amount(addrOut.Value).ToCoin()
	...
	return &AddressInfo{
		...
		AmountReceived:  cdrutil.Amount(received),
		AmountSent:      cdrutil.Amount(sent),
		AmountUnspent:   cdrutil.Amount(received - sent),
	}
}
