	...
	keepon:
		...
		case hash, ok := <-p.blockChan:
			...
			if reorg {
				...
				// cdrsqlite's chainmonitor handles the reorg, but we keep going
				// to update the web UI with the new best block.
			}

			msgBlock, _ := p.collector.cdrChainSvr.GetBlock(hash)
			...
			chainHeight, err := p.collector.cdrChainSvr.GetBlockCount()
			if err != nil {
