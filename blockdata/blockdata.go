...
type BlockData struct {
	Header           cdrjson.GetBlockHeaderVerboseResult
	Connections      int32
	FeeInfo          cdrjson.FeeInfoBlock
	CurrentStakeDiff cdrjson.GetStakeDifficultyResult
	EstStakeDiff     cdrjson.EstimateStakeDiffResult
...
type Collector struct {
	mtx          sync.Mutex
	cdrChainSvr *rpcclient.Client
	netParams    *chaincfg.Params
	stakeDB      *stakedb.StakeDatabase
}

// NewCollector creates a new Collector.
func NewCollector(cdrChainSvr *rpcclient.Client, params *chaincfg.Params,
...
// it into the BlockDataBasic and StakeInfoExtended and cdrataapi types.
...
func (t *Collector) CollectBlockInfo(hash *chainhash.Hash) (*apitypes.BlockDataBasic,
	*cdrjson.FeeInfoBlock, *cdrjson.GetBlockHeaderVerboseResult,
	*apitypes.BlockExplorerExtraInfo, *wire.MsgBlock, error) {
	msgBlock, err := t.cdrChainSvr.GetBlock(hash)
	if err != nil {
	...
	coinSupply, err := t.cdrChainSvr.GetCoinSupply()
	if err != nil {
		log.Error("GetCoinSupply failed: ", err)
	}
	nbSubsidy, err := t.cdrChainSvr.GetBlockSubsidy(int64(msgBlock.Header.Height)+1, 5)
	if err != nil {
	...
	sdiff := cdrutil.Amount(header.SBits).ToCoin()

	blockHeaderResults, err := t.cdrChainSvr.GetBlockHeaderVerbose(hash)
	if err != nil {
		return nil, nil, nil, nil, nil, err
	}
...
func (t *Collector) CollectHash(hash *chainhash.Hash) (*BlockData, *wire.MsgBlock, error) {
	...
	// still running, or cdr may be mad.
	t.mtx.Lock()
	defer t.mtx.Unlock()
	...
	numConn, err := t.cdrChainSvr.GetConnectionCount()
	if err != nil {
	...
	blockdata := &BlockData{
		...
		FeeInfo:          *feeInfoBlock,
		CurrentStakeDiff: cdrjson.GetStakeDifficultyResult{CurrentStakeDifficulty: blockDataBasic.StakeDiff},
		EstStakeDiff:     cdrjson.EstimateStakeDiffResult{},
		PoolInfo:         blockDataBasic.PoolInfo,
		ExtraInfo:        *extra,
		PriceWindowNum:   int(height / winSize),
...
func (t *Collector) Collect() (*BlockData, *wire.MsgBlock, error) {
	...
	go func() {
		bestBlockHash, err := t.cdrChainSvr.GetBestBlockHash()
	...
	case <-time.After(time.Second * 10):
		log.Errorf("Timeout waiting for cdr.")
		return nil, nil, errors.New("Timeout")
	}

	// Stake difficulty
	stakeDiff, err := t.cdrChainSvr.GetStakeDifficulty()
	if err != nil {
		return nil, nil, err
	}

	// estimatestakediff
	estStakeDiff, err := t.cdrChainSvr.EstimateStakeDiff(nil)
	if err != nil {
