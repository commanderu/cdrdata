...
func init() {
	...
	cdrsqlite.UseLogger(rpcclientLogger)
...
func mainCore() int {
	...
	client, _, err := rpcutils.ConnectNodeRPC(cfg.cdrServ, cfg.cdrUser,
		cfg.cdrPass, cfg.cdrCert, cfg.DisableDaemonTLS)
	if err != nil {
	...
	// Sqlite output
	dbInfo := cdrsqlite.DBInfo{FileName: cfg.DBFileName}
	//sqliteDB, err := cdrsqlite.InitDB(&dbInfo)
	sqliteDB, cleanupDB, err := cdrsqlite.InitWiredDB(&dbInfo, nil, client,
