...
The `rebuilddb2` app is used for maintenance of cdrata's `cdrpg` database that
uses PostgreSQL to store a nearly complete record of the commanderu blockchain data.
...
[postgresql-tuning.conf](../../db/cdrpg/postgresql-tuning.conf) for tips.

## Installation

Be able to build cdrata (see [../../README.md](../../README.md#build-from-source)). In short:

* Install `dep`, the dependency management tool

      go get -u -v github.com/golang/dep/cmd/dep

* Clone the cdrata repository
...
* Build `rebuilddb2`
      ...
      cd $GOPATH/src/github.com/commanderu/cdrata/cmd/rebuilddb2
      go build
      # or to install cdrata and other tools into $GOPATH/bin:
...
bulk data imports. Namely, before normal cdrata operation, ensure that
`fsync=true` and other setting are adjusted for efficient queries.

## Details

Rebuilding the cdrata tables from scratch involves the following steps:
...
See [LICENSE](../../LICENSE) at the base of the cdrata repository.
