...
var user = flag.String("user", "cdr", "node RPC username")
var pass = flag.String("pass", "bananas", "node RPC password")
var cert = flag.String("cert", "cdr.cert", "node RPC TLS certificate (when notls=false)")
...
func mainCore() int {
	...
	blocks := make(map[int64]*cdrutil.Block)
	...
	for i := int64(0); i < height+1; i++ {
		...
		blocks[i] = cdrutil.NewBlock(msgBlock)
		...
		blockSummaries[i] = apitypes.BlockDataBasic{
			...
			StakeDiff:  cdrutil.Amount(header.SBits).ToCoin(),
	...
	for i := range blockSummaries {
		blockSummaries[i].PoolInfo.Value = cdrutil.Amount(poolValues[i]).ToCoin()
