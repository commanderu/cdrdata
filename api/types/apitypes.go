...
// much of the time, cdrata will be using the types in cdrjson, but others are
// defined here
...
type AddressTxRaw struct {
	...
	Version       int32                `json:"version"`
	Locktime      uint32               `json:"locktime"`
	Vin           []cdrjson.VinPrevOut `json:"vin"`
	Vout          []Vout               `json:"vout"`
	Confirmations int64                `json:"confirmations"`
	BlockHash     string               `json:"blockhash"`
...
type AddressTotals struct {
	...
	CoinsSpent   float64 `json:"cdr_spent"`
	CoinsUnspent float64 `json:"cdr_unspent"`
}

// BlockDataWithTxType adds an array of TxRawWithTxType to
// cdrjson.GetBlockVerboseResult to include the stake transaction type
type BlockDataWithTxType struct {
	*cdrjson.GetBlockVerboseResult
	Votes   []TxRawWithVoteInfo
	Tickets []cdrjson.TxRawResult
	Revs    []cdrjson.TxRawResult
}

// TxRawWithVoteInfo adds the vote info to cdrjson.TxRawResult
type TxRawWithVoteInfo struct {
	cdrjson.TxRawResult
	VoteInfo VoteInfo
}

// TxRawWithTxType adds the stake transaction type to cdrjson.TxRawResult
...
type Status struct {
	...
	cdrataVersion  string `json:"cdrata_version"`
}
...
type BlockExplorerExtraInfo struct {
	...
	NextBlockSubsidy *cdrjson.GetBlockSubsidyResult `json:"next_block_subsidy"`
}
...
type StakeDiff struct {
	cdrjson.GetStakeDifficultyResult
	Estimates        cdrjson.EstimateStakeDiffResult `json:"estimates"`
...
type StakeInfoExtended struct {
	Feeinfo          cdrjson.FeeInfoBlock `json:"feeinfo"`
