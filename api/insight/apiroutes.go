...
func NewInsightContext(client *rpcclient.Client, blockData *cdrpg.ChainDBRPC, JSONIndent string) *insightApiContext {
...
func (c *insightApiContext) getTransactions(w http.ResponseWriter, r *http.Request) {
	...
	if address != "" {
		...
		txsOutput := struct {
			Txs []*cdrjson.SearchRawTransactionsResult `json:"txs"`
		}{
			txs,
		}
