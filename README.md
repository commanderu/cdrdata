...
[![Build Status](https://img.shields.io/travis/commanderu/cdrata.svg)](https://travis-ci.org/commanderu/cdrata)
[![GitHub release](https://img.shields.io/github/release/commanderu/cdrata.svg)](https://github.com/commanderu/cdrata/releases)
[![Latest tag](https://img.shields.io/github/tag/commanderu/cdrata.svg)](https://github.com/commanderu/cdrata/tags)
[![Go Report Card](https://goreportcard.com/badge/github.com/commanderu/cdrata)](https://goreportcard.com/report/github.com/commanderu/cdrata)
[![ISC License](https://img.shields.io/badge/license-ISC-blue.svg)](http://copyfree.org)

The cdrata repository is a collection of golang packages and apps for [commanderu](https://www.commanderu.org/) data collection, storage, and presentation.

## Repository overview

```none
../cdrata              The cdrata daemon.
...
├── cdrataapi          Package cdrataapi for golang API clients.
├── db
│   ├── dbtypes         Package dbtypes with common data types.
│   ├── cdrpg           Package cdrpg providing PostgreSQL backend.
│   └── cdrsqlite       Package cdrsqlite providing SQLite backend.
...
* Running `cdr` (>=1.1.2) synchronized to the current best block on the network.
...
* Clone the cdrata repository. It **must** be cloned into the following directory.

      git clone https://github.com/commanderu/cdrata $GOPATH/src/github.com/commanderu/cdrata

* Fetch dependencies, and build the `cdrata` executable.

      cd $GOPATH/src/github.com/commanderu/cdrata
      dep ensure
      # build cdrata executable in workspace:
      go build
...
left by dep builds of dependencies such as cdrwallet. You may safely delete
...
folder as the `cdrata` executable.
...
If you intend to run cdrata in "full" mode (i.e. with the `--pg` switch), which
uses a PostgreSQL database backend, it is crucial that you configure your
PostgreSQL server for your hardware and the cdrata workload.

Read [postgresql-tuning.conf](./db/cdrpg/postgresql-tuning.conf) carefully for
...
```bash
cp sample-cdrata.conf cdrata.conf
```

Then edit cdrata.conf with your cdr RPC settings. See the output of
`cdrata --help` for a list of all options and their default values.

### Indexing the Blockchain

If cdrata has not previously been run with the PostgreSQL database backend, it
is necessary to perform a bulk import of blockchain data and generate table
indexes. *This will be done automatically by `cdrata`* on a fresh startup.

Alternatively, the PostgreSQL tables may also be generated with the `rebuilddb2`
command line tool:

* Create the cdrata user and database in PostgreSQL (tables will be created automatically).
* Set your PostgreSQL credentials and host in both `./cmd/rebuilddb2/rebuilddb2.conf`,
  and `cdrata.conf` in the location specified by the `appdata` flag.
...
Note that cdrata requires that [cdr](https://docs.commanderu.org/getting-started/user-guides/cdr-setup/) is running with optional indexes enabled.  By default these indexes are not turned on when cdr is installed.
...
Launch the cdrata daemon and allow the databases to process new blocks. Both
SQLite and PostgreSQL synchronization require about an hour the first time
cdrata is run, but they are done concurrently. On subsequent launches, only
blocks new to cdrata are processed.

```bash
./cdrata    # don't forget to configure cdrata.conf in the appdata folder!
```

Unlike cdrata.conf, which must be placed in the `appdata` folder or explicitly
set with `-C`, the "public" and "views" folders *must* be in the same folder as
the `cdrata` executable.

## cdrata daemon

The root of the repository is the `main` package for the cdrata app, which has
...
After cdrata syncs with the blockchain server via RPC, by default it will begin
...
Note that while cdrata can be started with HTTPS support, it is recommended to
...
takes additional time and tens of gigabytes of disk storage space. Thus, cdrata
...
cdrata may be started with the `--pg` switch.
...
| Header |  `/block/best/header` | `cdrjson.GetBlockHeaderVerboseResult` |
...
| Verbose block result | `/block/best/verbose` | `cdrjson.GetBlockVerboseResult` |

| Block X (block index) | Path | Type |
| --- | --- | --- |
| Summary | `/block/X` | `types.BlockDataBasic` |
| Stake info |  `/block/X/pos` | `types.StakeInfoExtended` |
| Header |  `/block/X/header` | `cdrjson.GetBlockHeaderVerboseResult` |
...
| Verbose block result | `/block/X/verbose` | `cdrjson.GetBlockVerboseResult` |

| Block H (block hash) | Path | Type |
| --- | --- | --- |
| Summary | `/block/hash/H` | `types.BlockDataBasic` |
| Stake info |  `/block/hash/H/pos` | `types.StakeInfoExtended` |
| Header |  `/block/hash/H/header` | `cdrjson.GetBlockHeaderVerboseResult` |
...
| Verbose block result | `/block/hash/H/verbose` | `cdrjson.GetBlockVerboseResult` |
...
| Current sdiff separately | `/stake/diff/current` | `cdrjson.GetStakeDifficultyResult` |
| Estimates separately | `/stake/diff/estimates` | `cdrjson.EstimateStakeDiffResult` |
...
| --- | --- | --- |
| The current agenda and its status | `/stake/vote/info` | `cdrjson.GetVoteInfoResult` |
...
to keep in mind that the mempool in your node (cdr) is not likely to be exactly
the same as other nodes' mempool.  Also, your mempool is cleared out when you
shutdown cdr.  So, if you have recently (e.g. after the start of the current
ticket price window) started cdr, your mempool _will_ be missing transactions
...
of the cdrata daemon, but may be called alone with rebuilddb.

### rebuilddb2

`rebuilddb2` is a CLI app used for maintenance of cdrata's `cdrpg` database
(a.k.a. DB v2) that uses PostgreSQL to store a nearly complete record of the
commanderu blockchain data. See the [README.md](./cmd/rebuilddb2/README.md) for
...
`package cdrataapi` defines the data types, with json tags, used by the JSON
...
converting from standard commanderu data types (e.g. `wire.MsgBlock`) are also
provided.
...
`github.com/commanderu/cdr/database` with an ffldb storage backend from
`github.com/commanderu/cdr/database/ffldb`.  It also makes use of the `stake.Node`
type from `github.com/commanderu/cdr/blockchain/stake`.  The `ChainMonitor` type
...
`cdrutil.Tx`, `cdrutil.Block`, `chainhash.Hash`, and others.

## Internal-use packages

Packages `blockdata` and `cdrsqlite` are currently designed only for internal
use internal use by other cdrata packages, but they may be of general value in
the future.
...
* The `ChainDB` type, which is the primary exported type from `cdrpg`, providing
...
* The `wiredDB` type, intended to satisfy the `DataSourceLite` interface used by
  the cdrata app's API. The block header is not stored in the DB, so a RPC
...
See the GitHub issue tracker and the [project milestones](https://github.com/commanderu/cdrata/milestones).
...
1. Create a [pull request](https://github.com/commanderu/cdrata/compare).
...
Note that all cdrata.org community and team members are expected to adhere to
the code of conduct, described in the CODE_OF_CONDUCT file.

Also, [come chat with us on Slack](https://join.slack.com/t/cdrata/shared_invite/enQtMjQ2NzAzODk0MjQ3LTRkZGJjOWIyNDc0MjBmOThhN2YxMzZmZGRlYmVkZmNkNmQ3MGQyNzAxMzJjYzU1MzA2ZGIwYTIzMTUxMjM3ZDY)!
