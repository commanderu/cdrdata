...
type BlockGetter interface {
	...
	BestBlock() (*cdrutil.Block, error)
	Block(chainhash.Hash) (*cdrutil.Block, error)
...
// directly from cdr via RPC and subsequently update the internal block cache
// with the retrieved block.
type MasterBlockGetter interface {
	BlockGetter
	UpdateToBestBlock() (*cdrutil.Block, error)
	UpdateToNextBlock() (*cdrutil.Block, error)
	UpdateToBlock(height int64) (*cdrutil.Block, error)
}
...
type BlockGate struct {
	...
	blockWithHash map[chainhash.Hash]*cdrutil.Block
...
func NewBlockGate(client *rpcclient.Client, capacity int) *BlockGate {
	return &BlockGate{
		...
		blockWithHash: make(map[chainhash.Hash]*cdrutil.Block),
...
// NodeHeight gets the chain height from cdr.
...
func (g *BlockGate) BestBlock() (*cdrutil.Block, error) {
	g.RLock()
	defer g.RUnlock()
	var err error
	hash, ok := g.hashAtHeight[g.height]
...
func (g *BlockGate) Block(hash chainhash.Hash) (*cdrutil.Block, error) {
	g.RLock()
	defer g.RUnlock()
	var err error
	block, ok := g.blockWithHash[hash]
...
func (g *BlockGate) UpdateToBestBlock() (*cdrutil.Block, error) {
	_, height, err := g.client.GetBestBlock()
	if err != nil {
...
// updates the cache.
func (g *BlockGate) UpdateToNextBlock() (*cdrutil.Block, error) {
	g.Lock()
	height := g.height + 1
	g.Unlock()
	return g.UpdateToBlock(height)
...
// cdr and stores it in cache.
func (g *BlockGate) UpdateToBlock(height int64) (*cdrutil.Block, error) {
	g.Lock()
	defer g.Unlock()
	return g.updateToBlock(height)
}
