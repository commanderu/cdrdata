...
// ConnectNodeRPC attempts to create a new websocket connection to a cdr node,
...
func ConnectNodeRPC(host, user, pass, cert string, disableTLS bool,
	...
	if !disableTLS {
		cdrCerts, err = ioutil.ReadFile(cert)
		if err != nil {
			log.Errorf("Failed to read cdr cert file at %s: %s\n",
				cert, err.Error())
			return nil, nodeVer, err
		}
		log.Debugf("Attempting to connect to cdr RPC %s as user %s "+
			"using certificate located in %s",
			host, user, cert)
	} else {
		log.Debugf("Attempting to connect to cdr RPC %s as user %s (no TLS)",
			host, user)
	}
	...
	cdrClient, err := rpcclient.New(connCfgDaemon, ntfnHdlrs)
	if err != nil {
		return nil, nodeVer, fmt.Errorf("Failed to start cdr RPC client: %s", err.Error())
	}

	// Ensure the RPC server has a compatible API version.
	ver, err := cdrClient.Version()
	if err != nil {
		log.Error("Unable to get RPC version: ", err)
		return nil, nodeVer, fmt.Errorf("unable to get node RPC version")
	}

	cdrVer := ver["cdrjsonrpcapi"]
	nodeVer = semver.NewSemver(cdrVer.Major, cdrVer.Minor, cdrVer.Patch)
	...
	return cdrClient, nodeVer, nil
}

// BuildBlockHeaderVerbose creates a *cdrjson.GetBlockHeaderVerboseResult from
...
func BuildBlockHeaderVerbose(header *wire.BlockHeader, params *chaincfg.Params,
	currentHeight int64, nextHash ...string) *cdrjson.GetBlockHeaderVerboseResult {
	if header == nil {
		return nil
	}
	...
	blockHeaderResult := cdrjson.GetBlockHeaderVerboseResult{
		...
		SBits:         cdrutil.Amount(header.SBits).ToCoin(),
...
// GetBlockHeaderVerbose creates a *cdrjson.GetBlockHeaderVerboseResult for the
...
func GetBlockHeaderVerbose(client *rpcclient.Client, params *chaincfg.Params,
	idx int64) *cdrjson.GetBlockHeaderVerboseResult {
...
// GetBlockVerbose creates a *cdrjson.GetBlockVerboseResult for the block index
...
func GetBlockVerbose(client *rpcclient.Client, params *chaincfg.Params,
	idx int64, verboseTx bool) *cdrjson.GetBlockVerboseResult {
	blockhash, err := client.GetBlockHash(idx)
	if err != nil {
		log.Errorf("GetBlockHash(%d) failed: %v", idx, err)
		return nil
...
// GetBlockVerboseByHash creates a *cdrjson.GetBlockVerboseResult for the
...
func GetBlockVerboseByHash(client *rpcclient.Client, params *chaincfg.Params,
	hash string, verboseTx bool) *cdrjson.GetBlockVerboseResult {
...
func GetStakeDiffEstimates(client *rpcclient.Client) *apitypes.StakeDiff {
	...
	stakeDiffEstimates := apitypes.StakeDiff{
		GetStakeDifficultyResult: cdrjson.GetStakeDifficultyResult{
...
func GetBlock(ind int64, client *rpcclient.Client) (*cdrutil.Block, *chainhash.Hash, error) {
...
func GetBlockByHash(blockhash *chainhash.Hash, client *rpcclient.Client) (*cdrutil.Block, error) {
	msgBlock, err := client.GetBlock(blockhash)
	if err != nil {
...
func GetTransactionVerboseByID(client *rpcclient.Client, txid string) (*cdrjson.TxRawResult, error) {
...
// address
func SearchRawTransaction(client *rpcclient.Client, count int, address string) ([]*cdrjson.SearchRawTransactionsResult, error) {
