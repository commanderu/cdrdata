...
const (
	defaultConfigFilename = "cdrata.conf"
	defaultLogFilename    = "cdrata.log"
...
var (
	defaultHomeDir           = cdrutil.AppDataDir("cdrata", false)
	defaultConfigFile        = filepath.Join(defaultHomeDir, defaultConfigFilename)
	defaultLogDir            = filepath.Join(defaultHomeDir, defaultLogDirname)
	defaultDataDir           = filepath.Join(defaultHomeDir, defaultDataDirname)
	cdrHomeDir              = cdrutil.AppDataDir("cdr", false)
	defaultDaemonRPCCertFile = filepath.Join(cdrHomeDir, "rpc.cert")
	...
	defaultDBFileName = "cdrata.sqlt.db"

	defaultPGHost   = "127.0.0.1:5432"
	defaultPGUser   = "cdrata"
	defaultPGPass   = ""
	defaultPGDBName = "cdrata"
)
...
type config struct {
	...
	DBFileName         string `long:"dbfile" description:"SQLite DB file name (default is cdrata.sqlt.db)."`
	...
	// EmailSubject string `long:"emailsubj" description:"Email subject. (default \"cdrataapi transaction notification\")"`

	// RPC client options
	cdrUser         string `long:"cdruser" description:"Daemon RPC user name"`
	cdrPass         string `long:"cdrpass" description:"Daemon RPC password"`
	cdrServ         string `long:"cdrserv" description:"Hostname/IP and port of cdr RPC server to connect to (default localhost:9109, testnet: localhost:19109, simnet: localhost:19556)"`
	cdrCert         string `long:"cdrcert" description:"File containing the cdr certificate file"`
...
var (
	defaultConfig = config{
		...
		cdrCert:           defaultDaemonRPCCertFile,
...
func loadConfig() (*config, error) {
	...
	// them.
	if cfg.cdrServ == "" {
		cfg.cdrServ = defaultHost + ":" + activeNet.JSONRPCClientPort
	}
...
// time of writing, cdr currently places blocks for testnet version 0 in the
