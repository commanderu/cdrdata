...
func (p *mempoolMonitor) TxHandler(client *rpcclient.Client) {
	...
	for {
		...
		case s, ok := <-p.newTxHash:
			...
			if err != nil {
				log.Errorf("Failed to get transaction (do you have --txindex with cdr?) %v: %v",
					s.Hash.String(), err)
				continue
			}
			...
			//s.Tree() == cdrutil.TxTreeRegular
			// See cdr/blockchain/stake/staketx.go for information about
			...
			case stake.TxTypeSStx:
				...
				log.Tracef("Received ticket purchase %v, price %v",
					tx.Hash(), cdrutil.Amount(price).ToCoin())
...
type MempoolData struct {
	...
	NumVotes          uint32
	NewTickets        uint32
	Ticketfees        *cdrjson.TicketFeeInfoResult
...
func NewMempoolDataCollector(cdrChainSvr *rpcclient.Client, params *chaincfg.Params) *mempoolDataCollector {
...
func (t *mempoolDataCollector) Collect() (*MempoolData, error) {
	...
	mempoolTickets, err := c.GetRawMempoolVerbose(cdrjson.GRMTickets)
	if err != nil {
		return nil, err
	}

	mempoolVotes, err := c.GetRawMempoolVerbose(cdrjson.GRMVotes)
	if err != nil {
		return nil, err
	}
