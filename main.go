...
import (
	...
	"github.com/commanderu/cdrata/api"
	"github.com/commanderu/cdrata/api/insight"
	"github.com/commanderu/cdrata/blockdata"
	"github.com/commanderu/cdrata/db/dbtypes"
	"github.com/commanderu/cdrata/db/cdrpg"
	"github.com/commanderu/cdrata/db/cdrsqlite"
	"github.com/commanderu/cdrata/explorer"
	"github.com/commanderu/cdrata/mempool"
	m "github.com/commanderu/cdrata/middleware"
	notify "github.com/commanderu/cdrata/notification"
	"github.com/commanderu/cdrata/rpcutils"
	"github.com/commanderu/cdrata/semver"
	"github.com/commanderu/cdrata/txhelpers"
	"github.com/commanderu/cdrata/version"
...
func mainCore() error {
	...
	cfg, err := loadConfig()
	if err != nil {
		fmt.Printf("Failed to load cdrata config: %s\n", err.Error())
		return err
	}
	defer func() {
		if logRotator != nil {
	...
	// Connect to cdr RPC server using websockets
	...
	cdrClient, nodeVer, err := connectNodeRPC(cfg, ntfnHandlers)
	if err != nil || cdrClient == nil {
		return fmt.Errorf("Connection to cdr failed: %v", err)
	}
	...
	defer func() {
		...
		if cdrClient != nil {
			log.Infof("Closing connection to cdr.")
			cdrClient.Shutdown()
		}
	...
	curnet, err := cdrClient.GetCurrentNet()
	if err != nil {
		return fmt.Errorf("Unable to get current network from cdr: %v", err)
	}
	log.Infof("Connected to cdr (JSON-RPC API v%s) on %v",
	...
	dbInfo := cdrsqlite.DBInfo{FileName: dbPath}
	baseDB, cleanupDB, err := cdrsqlite.InitWiredDB(&dbInfo,
		notify.NtfnChans.UpdateStatusDBHeight, cdrClient, activeChain, cfg.DataDir)
	defer cleanupDB()
	if err != nil {
	...
	if usePG {
		...
		auxDB, err = cdrpg.NewChainDB(&dbi, activeChain, baseDB.GetStakeDB())
		if auxDB != nil {
			defer auxDB.Close()
		}
		if err != nil {
	...
	_, height, err := cdrClient.GetBestBlock()
	if err != nil {
	...
	collector := blockdata.NewCollector(cdrClient, activeChain, baseDB.GetStakeDB())
	if collector == nil {
	...
	getSyncd := func(updateAddys, updateVotes, newPGInds bool,
		...
		smartClient := rpcutils.NewBlockGate(cdrClient, 10)
	...
	// Register for notifications from cdr
	cerr := notify.RegisterNodeNtfnHandlers(cdrClient)
	if cerr != nil {
	...
	// On reorg, only update web UI since cdrsqlite's own reorg handler will
	...
	collectionQueue.SetSynchronousHandlers([]func(*chainhash.Hash){
		...
		wiredDBChainMonitor.BlockConnectedSync, // 3. cdrsqlite for sqlite DB reorg handling
	})
	...
	// cdrsqlite to do the switch, except for the last block which gets
	...
	// cdrsqlite does not handle new blocks except during reorg
	wg.Add(2)
	go wiredDBChainMonitor.BlockConnectedHandler()
	go wiredDBChainMonitor.ReorgHandler()

	if cfg.MonitorMempool {
		mpoolCollector := mempool.NewMempoolDataCollector(cdrClient, activeChain)
	...
	// Start web API
	app := api.NewContext(cdrClient, &baseDB, auxDB, cfg.IndentJSON)
	...
	if usePG {
		chainDBRPC, _ := cdrpg.NewChainDBRPC(auxDB, cdrClient)
		insightApp := insight.NewInsightContext(cdrClient, chainDBRPC, cfg.IndentJSON)
...
func waitForSync(base chan dbtypes.SyncResult, aux chan dbtypes.SyncResult,
	...
	if baseRes.Error != nil {
		log.Errorf("cdrsqlite.SyncDBAsync failed at height %d.", baseDBHeight)
		close(quit)
	...
	if useAux {
		...
		if auxRes.Error != nil {
			close(quit)
			if baseRes.Error != nil {
				log.Error("cdrsqlite.SyncDBAsync AND cdrpg.SyncChainDBAsync "+
			...
			log.Errorf("cdrpg.SyncChainDBAsync failed at height %d.", auxDBHeight)
...
func connectNodeRPC(cfg *config, ntfnHandlers *rpcclient.NotificationHandlers) (*rpcclient.Client, semver.Semver, error) {
	return rpcutils.ConnectNodeRPC(cfg.cdrServ, cfg.cdrUser, cfg.cdrPass,
		cfg.cdrCert, cfg.DisableDaemonTLS, ntfnHandlers)
}
...
func listenAndServeProto(listen, proto string, mux http.Handler) error {
	...
	if proto == "https" {
		go func() {
			errChan <- server.ListenAndServeTLS("cdrata.cert", "cdrata.key")
		}()
	} else {
		go func() {
