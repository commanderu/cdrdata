...
// NewChainDB constructs a ChainDB for the given connection and commanderu network
...
// DropTables drops (deletes) all of the known cdrata tables.
...
func (pgb *ChainDB) AddressHistory(address string, N, offset int64,
	...
	log.Infof("%s: %d spent totalling %f cdr, %d unspent totalling %f cdr",
		address, balanceInfo.NumSpent, cdrutil.Amount(balanceInfo.TotalSpent).ToCoin(),
		balanceInfo.NumUnspent, cdrutil.Amount(balanceInfo.TotalUnspent).ToCoin())
...
func (pgb *ChainDB) FillAddressTransactions(addrInfo *explorer.AddressInfo) error {
	...
	for _, txn := range addrInfo.Transactions {
		...
		txn.Total = cdrutil.Amount(dbTx.Sent).ToCoin()
...
func (pgb *ChainDB) AddressTotals(address string) (*apitypes.AddressTotals, error) {
	...
	return &apitypes.AddressTotals{
		...
		CoinsSpent:   cdrutil.Amount(ab.TotalSpent).ToCoin(),
		CoinsUnspent: cdrutil.Amount(ab.TotalUnspent).ToCoin(),
	}, nil
}
...
func (pgb *ChainDB) DeleteDuplicates() error {
	...
	if numVinsRemoved, err = pgb.DeleteDuplicateVins(); err != nil {
		return fmt.Errorf("cdrpg.DeleteDuplicateVins failed: %v", err)
	}
	...
	if numVoutsRemoved, err = pgb.DeleteDuplicateVouts(); err != nil {
		return fmt.Errorf("cdrpg.DeleteDuplicateVouts failed: %v", err)
	}
	...
	if numTxnsRemoved, err = pgb.DeleteDuplicateTxns(); err != nil {
		return fmt.Errorf("cdrpg.DeleteDuplicateTxns failed: %v", err)
	}
...
func (pgb *ChainDB) DeleteDuplicatesRecovery() error {
	...
	if numTxnsRemoved, err = pgb.DeleteDuplicateTickets(); err != nil {
		return fmt.Errorf("cdrpg.DeleteDuplicateTickets failed: %v", err)
	}
	...
	if numTxnsRemoved, err = pgb.DeleteDuplicateVotes(); err != nil {
		return fmt.Errorf("cdrpg.DeleteDuplicateVotes failed: %v", err)
	}
	...
	if numTxnsRemoved, err = pgb.DeleteDuplicateMisses(); err != nil {
		return fmt.Errorf("cdrpg.DeleteDuplicateMisses failed: %v", err)
	}
