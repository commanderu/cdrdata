...
import (
	...
	"github.com/commanderu/cdrata/db/cdrpg/internal"
...
func InsertTickets(db *sql.DB, dbTxns []*dbtypes.Tx, txDbIDs []uint64, checked bool) ([]uint64, []*dbtypes.Tx, error) {
	...
	for i, tx := range ticketTx {
		...
		price := cdrutil.Amount(tx.Vouts[0].Value).ToCoin()
		fee := cdrutil.Amount(tx.Fees).ToCoin()
		isSplit := tx.NumVin > 1
...
func InsertVotes(db *sql.DB, dbTxns []*dbtypes.Tx, _ /*txDbIDs*/ []uint64,
	...
	for i, tx := range voteTxs {
		...
		stakeSubmissionAmount := cdrutil.Amount(msgTx.TxIn[1].ValueIn).ToCoin()
		...
		voteReward := cdrutil.Amount(msgTx.TxIn[0].ValueIn).ToCoin()
