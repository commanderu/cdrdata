...
// parameters, and a status update channel. It calls cdrsqlite.NewDB to create a
...
func (db *wiredDB) GetHeader(idx int) *cdrjson.GetBlockHeaderVerboseResult {
	return rpcutils.GetBlockHeaderVerbose(db.client, db.params, int64(idx))
}

func (db *wiredDB) GetBlockVerbose(idx int, verboseTx bool) *cdrjson.GetBlockVerboseResult {
	return rpcutils.GetBlockVerbose(db.client, db.params, int64(idx), verboseTx)
}

func (db *wiredDB) GetBlockVerboseByHash(hash string, verboseTx bool) *cdrjson.GetBlockVerboseResult {
...
func (db *wiredDB) GetBlockSubsidy(height int64, voters uint16) *cdrjson.GetBlockSubsidyResult {
...
func (db *wiredDB) GetAllTxIn(txid string) []*apitypes.TxIn {
	...
	for i := range allTxIn {
		txIn := &apitypes.TxIn{
			...
			ValueIn:         cdrutil.Amount(allTxIn0[i].ValueIn).ToCoin(),
...
func (db *wiredDB) GetAllTxOut(txid string) []*apitypes.TxOut {
	...
	for i := range allTxOut {
		...
		txOut := &apitypes.TxOut{
			Value:     cdrutil.Amount(allTxOut0[i].Value).ToCoin(),
...
func (db *wiredDB) DecodeRawTransaction(txhex string) (*cdrjson.TxRawResult, error) {
...
func (db *wiredDB) getRawTransaction(txid string) (*apitypes.Tx, string) {
	...
	tx.Vin = make([]cdrjson.Vin, len(txraw.Vin))
...
// GetVoteVersionInfo requests stake version info from the cdr RPC server
func (db *wiredDB) GetVoteVersionInfo(ver uint32) (*cdrjson.GetVoteInfoResult, error) {
...
func (db *wiredDB) GetStakeVersions(txHash string, count int32) (*cdrjson.GetStakeVersionsResult, error) {
...
func (db *wiredDB) GetStakeVersionsLatest() (*cdrjson.StakeVersions, error) {
...
// on the stake version with which cdrata is compiled with (chaincfg.Params),
...
func (db *wiredDB) GetFeeInfo(idx int) *cdrjson.FeeInfoBlock {
...
func (db *wiredDB) GetAddressTransactionsRawWithSkip(addr string, count int, skip int) []*apitypes.AddressTxRaw {
	...
	for i := range txs {
		...
		tx.Vin = make([]cdrjson.VinPrevOut, len(txs[i].Vin))
...
func makeExplorerBlockBasic(data *cdrjson.GetBlockVerboseResult) *explorer.BlockBasic {
...
func makeExplorerTxBasic(data cdrjson.TxRawResult, msgTx *wire.MsgTx, params *chaincfg.Params) *explorer.TxBasic {
...
func makeExplorerAddressTx(data *cdrjson.SearchRawTransactionsResult, address string) *explorer.AddressTx {
...
func (db *wiredDB) GetExplorerBlock(hash string) *explorer.BlockInfo {
	...
	getTotalFee := func(txs []*explorer.TxBasic) (total cdrutil.Amount) {
		for _, tx := range txs {
			total += tx.Fee
		}
		return
	}
	getTotalSent := func(txs []*explorer.TxBasic) (total cdrutil.Amount) {
		for _, tx := range txs {
			amt, err := cdrutil.NewAmount(tx.Total)
			if err != nil {
				continue
			}
			total += amt
...
func (db *wiredDB) GetExplorerAddress(address string, count, offset int64) *explorer.AddressInfo {
	addr, err := cdrutil.DecodeAddress(address)
	if err != nil {
	...
	var totalreceived, totalsent cdrutil.Amount
		...
		for _, u := range tx.Vin {
			if u.PrevOut != nil && len(u.PrevOut.Addresses) != 0 {
				if address == u.PrevOut.Addresses[0] {
					t, _ := cdrutil.NewAmount(*u.AmountIn)
					if t > 0 {
						totalsent += t
					}
					numSpending++
...
func ValidateNetworkAddress(address cdrutil.Address, p *chaincfg.Params) bool {
...
func (db *wiredDB) UnconfirmedTxnsForAddress(address string) (*txhelpers.AddressOutpoints, int64, error) {
	...
	mempoolTxns, err := db.client.GetRawMempool(cdrjson.GRMAll)
	if err != nil {
...
func (db *wiredDB) GetMempool() []explorer.MempoolTx {
	mempooltxs, err := db.client.GetRawMempoolVerbose(cdrjson.GRMAll)
	if err != nil {
