...
import (
	...
	_ "github.com/commanderu/cdr/database/ffldb" // init the ffldb driver
...
func BuildStakeTree(blocks map[int64]*cdrutil.Block, netParams *chaincfg.Params,
...
func TicketsInBlock(bl *cdrutil.Block) ([]chainhash.Hash, []*wire.MsgTx) {
...
func TicketTxnsInBlock(bl *cdrutil.Block) ([]chainhash.Hash, []*cdrutil.Tx) {
	tickets := make([]chainhash.Hash, 0)
	ticketTxns := make([]*cdrutil.Tx, 0)
	for _, stx := range bl.STransactions() {
		if stake.DetermineTxType(stx.MsgTx()) == stake.TxTypeSStx {
			h := stx.Hash()
			tickets = append(tickets, *h)
...
func VotesInBlock(bl *cdrutil.Block) []chainhash.Hash {
