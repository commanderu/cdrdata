...
type TxGetter struct {
	txLookup map[chainhash.Hash]*cdrutil.Tx
}

func (t TxGetter) GetRawTransaction(txHash *chainhash.Hash) (*cdrutil.Tx, error) {
...
func LoadTestBlockAndSSTX(t *testing.T) (*cdrutil.Block, []*cdrutil.Tx) {
	// Load block data
	...
	block, err := cdrutil.NewBlockFromReader(blockTestFile)
	if err != nil {
	...
	allTxRead := make([]*cdrutil.Tx, numSSTX)
	for i := range allTxRead {
		...
		allTxRead[i], err = cdrutil.NewTxFromReader(txFile)
		if err != nil {
			t.Fatal(err)
		}
...
func TestFeeRateInfoBlock(t *testing.T) {
	...
	fibExpected := cdrjson.FeeInfoBlock{
		Height: 138883,
		Number: 20,
...
func TxToWriter(tx *cdrutil.Tx, w io.Writer) error {
	msgTx := tx.MsgTx()
...
func ConnectNodeRPC(host, user, pass, cert string, disableTLS bool) (*rpcclient.Client, semver.Semver, error) {
	...
	cdrClient, err := rpcclient.New(connCfgDaemon, nil)
	if err != nil {
