...
type RawTransactionGetter interface {
	GetRawTransaction(txHash *chainhash.Hash) (*cdrutil.Tx, error)
}
...
type VerboseTransactionGetter interface {
	GetRawTransactionVerbose(txHash *chainhash.Hash) (*cdrjson.TxRawResult, error)
}
...
type BlockWatchedTx struct {
	BlockHeight   int64
	TxsForAddress map[string][]*cdrutil.Tx
}
...
// TxhashInSlice searches a slice of *cdrutil.Tx for a transaction with the hash
// txHash. If found, it returns the corresponding *Tx, otherwise nil.
func TxhashInSlice(txs []*cdrutil.Tx, txHash *chainhash.Hash) *cdrutil.Tx {
	if len(txs) < 1 {
		return nil
	}
...
func IncludesStakeTx(txHash *chainhash.Hash, block *cdrutil.Block) (int, int8) {
...
func IncludesTx(txHash *chainhash.Hash, block *cdrutil.Block) (int, int8) {
...
func BlockConsumesOutpointWithAddresses(block *cdrutil.Block, addrs map[string]TxAction,
	c RawTransactionGetter, params *chaincfg.Params) map[string][]*cdrutil.Tx {
	addrMap := make(map[string][]*cdrutil.Tx)

	checkForOutpointAddr := func(blockTxs []*cdrutil.Tx) {
					...
					for _, txAddr := range txAddrs {
						...
						if _, ok := addrs[addrstr]; ok {
							if addrMap[addrstr] == nil {
								addrMap[addrstr] = make([]*cdrutil.Tx, 0)
							}
...
// specified addresses, and creates a map of addresses to a slice of cdrutil.Tx
// involving the address.
func BlockReceivesToAddresses(block *cdrutil.Block, addrs map[string]TxAction,
	params *chaincfg.Params) map[string][]*cdrutil.Tx {
	addrMap := make(map[string][]*cdrutil.Tx)

	checkForAddrOut := func(blockTxs []*cdrutil.Tx) {
		for _, tx := range blockTxs {
			// Check the addresses associated with the PkScript of each TxOut
				...
				for _, txAddr := range txOutAddrs {
					...
					if _, ok := addrs[addrstr]; ok {
						if _, gotSlice := addrMap[addrstr]; !gotSlice {
							addrMap[addrstr] = make([]*cdrutil.Tx, 0) // nil
						}
...
func MedianAmount(s []cdrutil.Amount) cdrutil.Amount {
	if len(s) == 0 {
		return 0
	}
...
// MedianCoin gets the median cdr from a slice of float64s
...
func SSTXInBlock(block *cdrutil.Block) []*cdrutil.Tx {
...
func SSGenVoteBlockValid(msgTx *wire.MsgTx) (BlockValidation, uint16, error) {
	...
	blockValid := BlockValidation{
		...
		Validity: cdrutil.IsFlagSet16(ssGenVoteBits, cdrutil.BlockValid),
	}
	return blockValid, ssGenVoteBits, nil
}

// VoteBitsInBlock returns a list of vote bits for the votes in a block
func VoteBitsInBlock(block *cdrutil.Block) []stake.VoteVersionTuple {
...
// specified block.
func FeeInfoBlock(block *cdrutil.Block) *cdrjson.FeeInfoBlock {
	feeInfo := new(cdrjson.FeeInfoBlock)
	...
	for it, msgTx := range sstxMsgTxns {
		...
		fee := cdrutil.Amount(amtIn - amtOut).ToCoin()
		if fee < minFee {
			minFee = fee
		}
		if fee > maxFee {
...
func FeeRateInfoBlock(block *cdrutil.Block) *cdrjson.FeeInfoBlock {
	feeInfo := new(cdrjson.FeeInfoBlock)
	_, sstxMsgTxns := TicketsInBlock(block)
	...
	for it, msgTx := range sstxMsgTxns {
		...
		fee := cdrutil.Amount(1000*(amtIn-amtOut)).ToCoin() / float64(msgTx.SerializeSize())
		if fee < minFee {
			minFee = fee
		}
		if fee > maxFee {
...
func TxFee(msgTx *wire.MsgTx) cdrutil.Amount {
	var amtIn int64
	for iv := range msgTx.TxIn {
		amtIn += msgTx.TxIn[iv].ValueIn
	}
	...
	return cdrutil.Amount(amtIn - amtOut)
}

// TxFeeRate computes and returns the fee rate in cdr/KB for a given tx
func TxFeeRate(msgTx *wire.MsgTx) (cdrutil.Amount, cdrutil.Amount) {
	var amtIn int64
	for iv := range msgTx.TxIn {
		amtIn += msgTx.TxIn[iv].ValueIn
	}
	...
	return cdrutil.Amount(amtIn - amtOut), cdrutil.Amount(1000 * (amtIn - amtOut) / int64(msgTx.SerializeSize()))
}

// TotalOutFromMsgTx computes the total value out of a MsgTx
func TotalOutFromMsgTx(msgTx *wire.MsgTx) cdrutil.Amount {
	var amtOut int64
	for _, v := range msgTx.TxOut {
		amtOut += v.Value
	}
	return cdrutil.Amount(amtOut)
}

// TotalVout computes the total value of a slice of cdrjson.Vout
func TotalVout(vouts []cdrjson.Vout) cdrutil.Amount {
	var total cdrutil.Amount
	for _, v := range vouts {
		a, err := cdrutil.NewAmount(v.Value)
		if err != nil {
			continue
		}
		total += a
