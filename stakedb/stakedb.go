// Copyright (c) 2018, The cdrata developers.
...
import (
	...
	"github.com/commanderu/cdr/database"
...
type StakeDatabase struct {
	...
	BestNode        *stake.Node
	blkMtx          sync.RWMutex
	blockCache      map[int64]*cdrutil.Block
...
func NewStakeDatabase(client *rpcclient.Client, params *chaincfg.Params,
	...
	sDB := &StakeDatabase{
		params:          params,
		NodeClient:      client,
		blockCache:      make(map[int64]*cdrutil.Block),
...
func (db *StakeDatabase) block(ind int64) (*cdrutil.Block, bool) {
	db.blkMtx.RLock()
...
func (db *StakeDatabase) ConnectBlockHash(hash *chainhash.Hash) (*cdrutil.Block, error) {
...
func (db *StakeDatabase) ConnectBlock(block *cdrutil.Block) error {
	height := block.Height()
...
func (db *StakeDatabase) connectBlock(block *cdrutil.Block, spent []chainhash.Hash,
...
func (db *StakeDatabase) Open(dbName string) error {
	...
	if err != nil {
		if strings.Contains(err.Error(), "resource temporarily unavailable") ||
			...
			return fmt.Errorf("Stake DB already opened. cdrata running?")
		}
...
func (db *StakeDatabase) calcPoolInfo(liveTickets, winningTickets []chainhash.Hash, height uint32) *apitypes.TicketPoolInfo {
	...
	poolCoin := cdrutil.Amount(poolValue).ToCoin()
	valAvg := 0.0
...
// DBTipBlock gets the cdrutil.Block for the current best block in the stake
// database. It used DBState to get the best block hash, and the node RPC client
// to get the block itself.
func (db *StakeDatabase) DBTipBlock() (*cdrutil.Block, error) {
	_, hash, err := db.DBState()
	if err != nil {
		return nil, err
	}
...
// DBPrevBlock gets the cdrutil.Block for the previous best block in the stake
// database. It used DBState to get the best block hash, and the node RPC client
// to get the block itself.
func (db *StakeDatabase) DBPrevBlock() (*cdrutil.Block, error) {
	_, hash, err := db.DBState()
	if err != nil {
		return nil, err
	}
...
func (db *StakeDatabase) getBlock(hash *chainhash.Hash) (*cdrutil.Block, error) {
	msgBlock, err := db.NodeClient.GetBlock(hash)
	if err == nil {
		return cdrutil.NewBlock(msgBlock), nil
	}
	return nil, err
}
